...
@router.post("/calendar/add-entry", response_model=CalendarEntry, tags=["calendar"])
def add_calendar_entry(calendar_entry: CalendarEntry, current_user: User = Depends(get_current_user)):
    try:
...
@router.put("/calendar/update-entry/{entry_id}", response_model=CalendarEntry, tags=["calendar"])
def update_calendar_entry(entry_id: str, calendar_entry: CalendarEntry, current_user: User = Depends(get_current_user)):
    try:
...
async def upload_file(file: UploadFile = File(...)):
    try:
        ...
        file_ref = FileReference(file_id=result.inserted_id, filename=file.filename)
        return file_ref
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
