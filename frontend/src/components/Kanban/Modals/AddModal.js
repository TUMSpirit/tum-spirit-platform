...
import { getRandomColors } from "../helpers/getRandomColors"; // If needed, adjust import for random colors
import { v4 as uuidv4 } from "uuid";
import { Button, Modal, Row, Col, Form, Input, message, Select, Tag } from "antd";
...
const AddModal = ({ onClose, isCreateEventOpen, isUpdateEventOpen, handleAddTask, handleEditTask, handleDeleteTask, initValues }) => {
	...
	useEffect(() => {
		if (initValues && isUpdateEventOpen) {
			...
			setMilestone(initValues.milestone);
		}
	}, [initValues, form]);
	...
	const handleTagChange = (tag, checked) => {
		const updatedTags = checked
			? [...taskData.tags, tag]
			: taskData.tags.filter(t => t.title !== tag.title);
